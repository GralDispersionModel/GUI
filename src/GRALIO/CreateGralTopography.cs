...
namespace GralIO
{
    ...
    public class CreateGralTopography
    {
        ...
        public bool CreateTopography(Main f)
        {
            ...
            return ReadTopoData(Main.GUISettings.TopoFileName);
        }
        ...
        public bool GetFilename()
        {
            ...
            })
            {
                try
                {
                    if (string.IsNullOrEmpty(Main.GUISettings.TopoFileName))
                    {
                        Main.GUISettings.TopoFileName = Main.ProjectName;
                    }
                    dialog1.InitialDirectory = Path.GetDirectoryName(Main.GUISettings.TopoFileName);
                    dialog1.FileName = Path.GetFileName(Main.GUISettings.TopoFileName);

                    if (dialog1.ShowDialog() == DialogResult.OK)
                    {
                        Main.GUISettings.TopoFileName = dialog1.FileName;
