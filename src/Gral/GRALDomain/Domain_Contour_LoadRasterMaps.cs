    ...
    public partial class Domain
	{
		...
		public void Contours(string file, DrawingObjects _drobj)
		{
			...
			if (ReDrawContours == true) // new contour line build blocked?
			{
				...
				int nx = 0;	int ny = 0;
				double x11 = 0;	double y11 = 0;	double dx = 0;
                ...
                try
				{
					...
					else // Textfile reader
					{
						...
						data = myReader.ReadLine().Split(new char[] { ' ', '\t', ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
						x11 = Convert.ToDouble(data[1], ic);
						data = myReader.ReadLine().Split(new char[] { ' ', '\t', ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
						y11 = Convert.ToDouble(data[1], ic);
						data = myReader.ReadLine().Split(new char[] { ' ', '\t', ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
						dx = Convert.ToDouble(data[1], ic);
						data = myReader.ReadLine().Split(new char[] { ' ', '\t', ',', ';' }, StringSplitOptions.RemoveEmptyEntries);
						...
						if (data.Length > 4) // Read if map is a vertical concentration profile
						{
							if (data[data.Length - 3].Contains("Vertical_concentration"))
							{
								...
								PointF pt1 = new PointF((float) x11, (float) y11);
								VTC_List.Add (pt1);
								PointF pt2 = new PointF((float) dx, (float) nodata);
								VTC_List.Add (pt2);
								PointF pt3 = new PointF((float) nx, (float) ny);
								VTC_List.Add (pt3);
								PointF pt4 = new PointF(max_h, 0);
								VTC_List.Add (pt4);
						...
						zlevel = new double[nx + 1, ny]; // nx + 1 -> add additional (empty) column for simple contour line algorithm
						
						zlevelfilter = new double[nx, ny];
						bool low_pass_filter = _drobj.Filter;
						...
						if (evaluate_terrain) // evaluate terrain for vertical concentration maps
						{
							for (int j = 0; j < nx; ++j)
							{
								...
								float _xh = (float) (x11 + (j) * dx); // coordintaes of this terrain point
								float _yh = (float) (y11 - (ny - terrain_index) * dx);
                ...
                if (_drobj.Filter == true)
                {
                    ReadBuildingIndex(buildingsindex, zlevelfilter, dx, x11, y11, nx, ny);
                }
                else
                {
                    ReadBuildingIndex(buildingsindex, zlevel, dx, x11, y11, nx, ny);
                }
                ...
                if (_drobj.Filter == true)
				{
					...
					Application.DoEvents();
					LowPassFilter(zlevel, zlevelfilter, buildingsindex, nx, ny, dx, x11, y11, nodata);				
				} 
				...
				if (_drobj.DrawSimpleContour)
				{
				    ...
				    SimpleContour(zlevel, nx, ny, x11, y11, dx, message, _drobj, buildingsindex);
				    message.Close();
				    Cursor = Cursors.Default;
		             					...
		             					if (!largeArray) // small arrays - 4 triangles
		             					{
		             						for (Int16 m = 4; m > -1; m--)
		             						{
		             							if (m > 0)
		             							{
		             								...
		             								xh[m] = (float) (x11 + (i + im[m - 1]) * dx);
		             								yh[m] = (float) (y11 + (j + jm[m - 1]) * dx);
		             							}
		             							else
		             							{
		             								h[0] = 0.25F * (h[1] + h[2] + h[3] + h[4]);
		             								xh[0] = (float) (0.5F * (x11 + (i) * dx + x11 + (i + 1) * dx));
						...
						Parallel.For(0, _drobj.ItemValues.Count, k =>
			             {
			             	...
			             	if (unsorted[k].Count > 0)
			             	{
			             		...
			             		while (unsorted[k].Count > 0)
			             		{
			             			...
			             			{
			             				if (Math.Abs(xA - unsorted[k][i].X) < 0.01 && Math.Abs(yA - unsorted[k][i].Y) < 0.01)
			             				{
			             					_contPts[k].Add(new PointF(unsorted[k][i].X , unsorted[k][i].Y));
			             					xA = unsorted[k][i].Width;
			             					yA = unsorted[k][i].Height;
			             					_contPts[k].Add(new PointF(xA , yA));
			             					area +=  ((xA - unsorted[k][i].X) * unsorted[k][i].Y + (xA - unsorted[k][i].X) * (yA - unsorted[k][i].Y) / 2);
			             				...
			             				else if (Math.Abs(xA - unsorted[k][i].Width ) < 0.01 && Math.Abs(yA - unsorted[k][i].Height) < 0.01)
			             				{
			             					...
			             					yA = unsorted[k][i].Y;
			             					_contPts[k].Add(new PointF(xA , yA));
			             					area +=  ((xA - unsorted[k][i].Width) * unsorted[k][i].Height + (xA - unsorted[k][i].Width) * (yA - unsorted[k][i].Height) / 2);
						...
						if (((buildingsindex[i, j] != true) || (zlevelfilter[i, j] != 0)) && Math.Abs(zlevelfilter[i, j] - nodata) > 0.001)
	             		{
							...
							zlevel[i, j] *= 1.0 / weighting_factor;
