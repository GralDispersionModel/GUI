...
namespace GralBackgroundworkers
{
    public partial class ProgressFormBackgroundworker
    {
                          ...
                          System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            float[][][] concmit = CreateArray<float[][]>(mydata.CellsGralX + 1, () => CreateArray<float[]>(mydata.CellsGralY + 1, () => new float[maxsource]));
            float[][][] depmit = CreateArray<float[][]>(mydata.CellsGralX + 1, () => CreateArray<float[]>(mydata.CellsGralY + 1, () => new float[maxsource]));
            ...
            if (nnn > 0)
            {
                ...
                foreach (string source_group_name in sg_names)
                {
                    ...
                    if (mydata.Checkbox2 == true)
                    {
                        for (int i = 0; i <= mydata.CellsGralX; i++)
                        {
                            ...
                            {
                                concmit[i][j][itm] = concmit[i][j][itm] / _number;
                                depmit[i][j][itm] = depmit[i][j][itm] / _number;
                            }
                        }
                    }
                    itm++;
                ...
                if (mydata.Checkbox2 == true)
                {
                    ...
                    for (int i = 0; i <= mydata.CellsGralX; i++)
                    {
                        for (int j = 0; j <= mydata.CellsGralY; j++)
                        {
                            concmit[i][j][maxsource] = concmit[i][j][maxsource] / _number;
                            depmit[i][j][maxsource] = depmit[i][j][maxsource] / _number;
                        }
                    }
                }
            }
