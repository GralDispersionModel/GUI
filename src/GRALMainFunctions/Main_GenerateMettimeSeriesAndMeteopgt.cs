...
namespace Gral
{
    ...
    partial class Main
    {
        ...
        private void GRALGenerateMeteoFiles(object sender, EventArgs e)
        {
            ...
            if (textBoxGrammTerrain.Text.Length > 0 && File.Exists(newPath))
            {
        ...
        private void GRALGenerateMeteoFilesForERA5()
        {
            ...
            {
                if (textBoxGrammTerrain.Text.Length > 0 && File.Exists(Path.Combine(ProjectName, "Computation", "meteopgt.all")))
                {
