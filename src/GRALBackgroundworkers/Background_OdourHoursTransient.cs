            ...
            try
            {
                for (int i = 0; i < hour.Count; i++)
                {
                    ...
                    Parallel.For(0, sg_names.Length, itmp =>
                    {
                        ...
                        {
                            con_files[itmp] = Convert.ToString(i + 1).PadLeft(5, '0') + "-" + Convert.ToString(mydata.Slice) + Gral.SourceGroupFileName.Encode(sg_numbers[itmp]) + ".con";
                            odr_files[itmp] = Convert.ToString(i + 1).PadLeft(5, '0') + "-" + Convert.ToString(mydata.Slice) + Gral.SourceGroupFileName.Encode(sg_numbers[itmp]) + ".odr";
                        }
                        else
                        {
                            con_files[itmp] = Convert.ToString(i + 1).PadLeft(5, '0') + "-" + Convert.ToString(mydata.Slice) + Gral.SourceGroupFileName.Encode(sg_names[itmp]) + ".con";
                            odr_files[itmp] = Convert.ToString(i + 1).PadLeft(5, '0') + "-" + Convert.ToString(mydata.Slice) + Gral.SourceGroupFileName.Encode(sg_names[itmp]) + ".odr";
                        }
